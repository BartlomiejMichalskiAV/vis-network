...
class Edge {
  ...
  static parseOptions(parentOptions, newOptions, allowDeletion = false, globalOptions = {}, copyFromGlobals = false) {
    ...
    } else if (!ComponentUtil.isValidLabel(parentOptions.labelFrom)) {
    ...
    } else if (!ComponentUtil.isValidLabel(parentOptions.labelTo)) {
      parentOptions.labelTo = undefined;
    }

    const mergeTriple = (a,b,c) => {
      if(a !== undefined) {
        return a;
      }
      if(b !== undefined) {
    ...
    parentOptions.rotateText = mergeTriple(newOptions.rotateText, globalOptions.rotateText, false);
    parentOptions.textAboveEdge = mergeTriple(newOptions.textAboveEdge, globalOptions.textAboveEdge, false);
    parentOptions.distanceFromNode = mergeTriple(newOptions.distanceFromNode, globalOptions.distanceFromNode, 20);
  ...
  drawLabel(ctx, viaNode) {
    if (this.options.label !== undefined) {
      this._drawLabel(ctx, viaNode, 0.5);
    } else {
      var node1 = this.from;
      ...
      var point = this._pointOnCircle(x, y, radius, 0.125);
      this.labelModule.draw(ctx, point.x, point.y, this.selected, this.hover);
    }

    if (this.options.labelFrom !== undefined && this.options.labelFrom !== "") {
      var saved = this.options.label;
      this.options.label = this.options.labelFrom;
      this.updateLabelModule();
      this._drawLabel(ctx, viaNode, 0);
      this.options.label = saved;
      this.updateLabelModule();
    }

    if (this.options.labelTo !== undefined && this.options.labelTo !== "") {
      var saved = this.options.label;
      this.options.label = this.options.labelTo;
      this.updateLabelModule();
      this._drawLabel(ctx, viaNode, 1);
      this.options.label = saved;
      this.updateLabelModule();
    }
  }

  getPointForLabel(position, distance) {
    const newPosition = {
      x: (1 - position) * this.from.x + position * this.to.x,
      y: (1 - position) * this.from.y + position * this.to.y
    };
    ...
    newPosition.y = newPosition.y - dy;
    return newPosition;
  }

  _drawLabel(ctx, viaNode, offset) {
    ...
    if (node1.id != node2.id) {
      ...
      var point = this.edgeType.getPoint(offset-0.05, viaNode);
      var point2 = this.edgeType.getPoint(offset+0.05, viaNode);
      ctx.save();

      if (this.options.rotateText && !this.options.smooth.enabled) {
        let rotationPoint = this._getRotation(ctx);
        // override always 0deg angle
        rotationPoint.angle = Math.atan2(point.y - point2.y, point.x - point2.x);
        
        // prevent text to display upside down
        if(rotationPoint.angle > Math.PI /2) {
          rotationPoint.angle = rotationPoint.angle - Math.PI;
        }
        if(rotationPoint.angle < - Math.PI /2) {
          rotationPoint.angle = rotationPoint.angle + Math.PI;
        }

        // make label sticky to node
        var labelSize = this.labelModule.size.width;
        var nodeSize = this.options.distanceFromNode;
        var offfsecik = offset <= 0.2 ? labelSize + nodeSize : ( offset >= 0.8 ? - (labelSize + nodeSize) : 0);

        var distance = Math.sqrt(Math.pow(this.from.x - this.to.x,2) + Math.pow(this.from.y - this.to.y,2));

        if(distance < Math.abs(offfsecik)) {
          offfsecik = 0;
          offset = 0.5;
        }

        var pointDraw = this.getPointForLabel(offset, offfsecik/2);
        ctx.translate(pointDraw.x, pointDraw.y);

        if (rotationPoint.angle != 0) {
          ctx.rotate(rotationPoint.angle);
          // avoid label and edge overlaping
          if (this.options.textAboveEdge) {
            ctx.translate(0, -this.labelModule.size.height/2);
          }
        }
        this.labelModule.draw(ctx, 0, 0, this.selected, this.hover);
      } else {
        var pointDraw = this.edgeType.getPoint(Math.min(Math.max(0.2, offset),0.8), viaNode);
        ctx.translate(pointDraw.x, pointDraw.y);
        if (this.options.textAboveEdge) {
          ctx.translate(0, -this.labelModule.size.height/2);
        }
