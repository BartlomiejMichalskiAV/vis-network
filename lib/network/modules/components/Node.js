...
class Node {
  ...
  draw(ctx) {
    ...
    if (this.options.status) {
      var get = (property, or) => {
        if(this.options.status.options) {
          return this.options.status.options[property] || or
        } else {
          return or;
        }
      }
      var offsetX = get('offsetX', 10);
      var offsetY = get('offsetY', 10);
      var size = get('size', 5);
      var color = get("color", "black");

      let oldStyle = ctx.fillStyle;
      let oldWidth = ctx.lineWidth;
      let oldborder = ctx.strokeStyle;
      

      ctx.beginPath()
      ctx.arc(this.x+offsetX, this.y + offsetY, size, 0, 2 * Math.PI, false);
      ctx.fillStyle = color;
      ctx.fill();
      ctx.closePath()
      ...
      ctx.strokeStyle = oldborder;
      
      if (this.options.status.options && this.options.status.options.label) {
        var getLabel = (property, or) => {
          if(this.options.status.options) {
            return this.options.status.options.label[property] || or
          } else {
            return or;
          }
        }
        const saved = this.options.label;
        const color = this.options.font.color;
        const textSize = this.options.font.size;
        const savedFill = ctx.fillStyle;
        const savedStroke = ctx.strokeStyle;
        this.options.label = getLabel('text', '');
        this.options.font.color = getLabel('color', 'gray');
        this.options.font.size = getLabel('size', this.options.font.size);
        this.updateLabelModule();
        this.labelModule.calculateLabelSize(ctx, false, false, this.x, this.y);
        const textOffset = getLabel('offset', 5);
        ctx.strokeStyle = getLabel('borderColor', 'gray');
        ctx.fillStyle = getLabel('backgroundColor', 'lightgray');
        const drawBg = getLabel("drawBackground", false);
        this.roundRect(ctx, this.x+offsetX + size + textOffset - 4, this.y + offsetY + -2 - this.labelModule.size.height / 2, this.labelModule.size.width + 8, this.labelModule.size.height + 4, 5, drawBg, true)
        this.labelModule.draw(ctx, this.x + offsetX + size + this.labelModule.size.width / 2 + textOffset , this.y + offsetY, false, false, 'middle', false)
        this.options.label = saved;
        this.options.font.color = color;
        this.options.font.size = textSize;
